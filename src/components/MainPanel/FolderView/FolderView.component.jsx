...
import { Table, OverlayTrigger, Popover, ListGroup, Modal, Button } from 'react-bootstrap';
...
import { fetchFolderDetails, fetchFolders, updateFolder, updateFolderDetails } from "../../../redux/actionCreator/folderActionCreator";
...
const FolderViewComponent = ({ folders }) => {
  ...
  const {id, name, path, subFolders, files, tempFolderId, tempFolderName, tempFolderPath } = useSelector((state) => ({
  ...
  const openFolder = useCallback((folder) => {
    ...
    dispatch(updateFolder(folder.id, folder))
  ...
  const handleMoveFolder = useCallback((moveFolder) => {
    moveFolder.parent = tempFolderName
  ...
  return (
    <>
      ...
      <div 
        className="shadow mt-3 me-3 p-3 pt-0 folder-view-scroll">        
      {
          ...
          }
          {
            ...
            <Modal centered show={showMoveToModal} backdrop="static" onHide={() => {setshowMoveToModal(false)}}>
              ...
              <Modal.Body>
                ...
                <FolderListComponent folders={folders} parent="" type="moveTo" removeFolderId={chosenFolder.id}/>
              </Modal.Body>
              <Modal.Footer>
